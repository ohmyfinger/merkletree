...
fn bench_get_merkle_tree_leafs(c: &mut Criterion) {
    for size in &[1, 256] {
        ...
        c.bench_function(&format!("get merkle-tree leafs {size}mib"), |b| {
            b.iter(|| get_merkle_tree_leafs(black_box(tree_size), DEFAULT_NUM_BRANCHES).unwrap());
        });
    }
}
...
fn bench_get_merkle_tree_info(c: &mut Criterion) {
    ...
    c.bench_function("get merkle-tree info 1gib", |b| {
        b.iter(|| {
            let tree_size =
                get_merkle_tree_len(black_box(sector_size), branches).expect("failed to get len");
            assert_eq!(
